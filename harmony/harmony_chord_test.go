...
func TestChordSymbolEsMoll(t *testing.T) {
	...
	tests := []struct {
		...
		{"IV7 第3転回位置", [4]uint8{75, 68, 59, 54}, "IV7(第3転回位置)"},           // Eb5 Ab4 Cb4 Gb3
		{"増六 完全形", [4]uint8{78, 69, 63, 59}, "V調のV9(増六・根音省略形・第2転回位置)"},    // Gb5 A4 Eb4 Cb4
		{"増六 第9音省略形", [4]uint8{75, 69, 63, 59}, "V調のV9(増六・根音省略形・第2転回位置)"}, // Eb5 A4 Eb4 Cb4
		{"ドリアのIV(+IV7) 基本位置", [4]uint8{66, 63, 60, 44}, "+IV7(基本位置)"},     // Gb4 Eb4 C4 Ab2
	}
	for _, tt := range tests {
		if got := chordSymbol(tt.notes, ts); got != tt.want {
			t.Errorf("%s: chordSymbol(%v) = %s, want %s", tt.name, tt.notes, got, tt.want)
